...
must_haves:
  truths:
    - "Preload exposes all 14 methods renderer.js actually calls via window.glassboard"
...
4. **Tests these describe blocks:**
   ...
   **describe('API completeness - all renderer.js calls are exposed'):**
   - Read `src/renderer.js` using `fs.readFileSync` (the REAL fs, not mocked) and extract all `window.glassboard.XXX` method names via regex `/window\.glassboard\.(\w+)/g`
   - For each extracted method name, verify `exposedAPI[methodName]` is defined and is a function
   - This is a programmatic contract test: if someone adds a new window.glassboard call in renderer.js, this test will fail until preload.js exposes it
   - Implementation: use `const fs = require('fs')` at top (before jest.mock), read renderer source, extract method names into a Set, then `test.each([...methodNames])('exposes %s', (name) => { expect(typeof exposedAPI[name]).toBe('function'); })`
